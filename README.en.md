...
That frustration is why I made **Panel 033**. It does not press a template onto your image. It first works out what must not be lost, then translates it into **Turn one photograph into a vivid, layered modern magazine-cover collage**.
...
These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 033's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.
...
/xxd-panel-033 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-033 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-033 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-033 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 033 to a whole directory; shared settings resolve once
/xxd-panel-033 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
...
- [Chinese runtime adapter](references/xxd-panel-033-prompt.zh-CN.md)
- [English runtime adapter](references/xxd-panel-033-prompt.en.md)
